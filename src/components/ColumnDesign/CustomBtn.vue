<template>
  <el-dialog :close-on-click-modal="false" class="JNPF-dialog JNPF-dialog_center form-script-dialog"
    ...
    <el-form class="form-script-dialog-body" ref="dataForm" :model="dataForm" :rules="dataRule"
      ...
      <div v-if="webType == 2" style="margin-bottom: 20px;">
        <el-form-item label="展示条件" style="margin-bottom: 10px;"></el-form-item>
        <el-table :data="dataForm.showConditionInfo">
          <el-table-column type="index" width="50" label="序号" align="center" />
          <el-table-column prop="field" label="表单字段">
            <template slot-scope="scope">
              <el-select v-model="scope.row.fieldName" placeholder="请选择" filterable>
          ...
          <el-table-column prop="field" label="等于" width="80" align="center">
            <template>等于</template>
          </el-table-column>
          <el-table-column prop="value" label="表单内容">
            <template slot-scope="scope">
              <el-input v-model="scope.row.fieldValue" placeholder="请输入表单内容"></el-input>
            </template>
          </el-table-column>
          <el-table-column label="操作" width="50">
            <template slot-scope="scope">
              <el-button size="mini" type="text" class="JNPF-table-delBtn"
                         @click="handleShowConditionDel(scope.$index)">删除
              </el-button>
            </template>
          </el-table-column>
        </el-table>
        <div class="table-actions" @click="addShowConditionOption" style="margin-bottom: 18px;">
          <el-button type="text" icon="el-icon-plus">新增</el-button>
        </div>
      </div>
      <el-form-item label-width="130px">
        <span slot="label" >展示到更多按钮
          <el-tooltip content="开启后，按钮将展示到更多按钮区内，否则将直接展示。只对于列表配置有用，其他页面配置无用" placement="top">
            <a class="el-icon-warning-outline"></a>
          </el-tooltip>
        </span>
        <el-switch v-model="dataForm.moreGroup" />
      </el-form-item>
      <el-form-item label-width="160px">
        <span slot="label" >是否先进行存储操作
          <el-tooltip content="开启后，在填写表单页面，会先执行数据保存操作，数据保存成功后才会执行所配置的脚本。只对于编辑表单页面有用，其他页面配置无用" placement="top">
            <a class="el-icon-warning-outline"></a>
          </el-tooltip>
        </span>
        <el-switch v-model="dataForm.isAfterSubmit" />
      </el-form-item>
      ...
      <template v-else-if="dataForm.btnType==2">
        <div class="right-main">
          ...
          <div class="tips">
            ...
            <p>data--列表当前行数据/详情当前页面数据，index--列表当前行下标</p>
            <p>request--异步请求，示例：request(url(接口地址，无需带入API Host),method(请求方式get, post),data(请求数据))</p>
            <p>toast--消息提示(ElMessage)，refresh--刷新页面，示例：refresh(isRefresh(是否刷新true、false))</p>
            <p>rowDataOperation--当前行数据操作，rowDataOperation(key(响应方式edit、detail、remove), data(当前操作数据), index(索引))</p>
          </div>
        </div>
      </template>
      <template v-else>
...
export default {
  ...
  data() {
    return {
      ...
      webType: "",
      columnOptions: [],
      defaultFunc: "({ data, index, request, toast, refresh }) => {\r\n   \r\n}"
    }
  },
  computed: {
    formFieldsOptions() {
  ...
  methods: {
    init(showType, activeItem, webType, columnOptions, defaultFunc) {
      this.webType = webType
      this.columnOptions = columnOptions || []
      this.showType = showType || 'pc'
      if (defaultFunc) this.defaultFunc = defaultFunc
      this.visible = true
      this.$nextTick(() => {
        this.resetData()
    ...
    confirm() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          if (this.dataForm.showConditionInfo.length !== 0) {
            if (this.dataForm.showConditionInfo.length) {
              for (let i = 0; i < this.dataForm.showConditionInfo.length; i++) {
                const element = this.dataForm.showConditionInfo[i];
                if (!element.fieldName) return this.$message({ message: `展示条件第${i + 1}行的表单字段不能为空`, type: 'warning' });
                if (!element.fieldValue) return this.$message({ message: `展示条件第${i + 1}行的表单内容不能为空`, type: 'warning' });
              }
            }
          }
    ...
    resetData() {
      ...
      this.dataForm.func = this.defaultFunc
      this.dataForm.interfaceId = ""
      this.dataForm.interfaceName = ""
      this.dataForm.templateJson = []
    ...
    addShowConditionOption() {
      this.dataForm.showConditionInfo.push({
        fieldName: '',
        fieldValue: '',
      })
    },
    handleShowConditionDel(index) {
      this.dataForm.showConditionInfo.splice(index, 1)
    },
  }
}
</script>
