...
<script>
import { cashToChinese, sumCash } from '@/utils/format'
...
export default {
        ...
        this.kArr.forEach(k => {
          if (k instanceof Array) {
            ...
            if (typeof k1 === 'object' && k1.key) {
              ...
              sumVal = sumCash(sumVal, _v);
            } else {
              (fd[k1] || []).forEach(v => {
                sumVal = sumCash(sumVal, v[k2]);
              });
            }
          } else {
            sumVal = sumCash(sumVal, fd[k]);
          }
        });
      } catch (e) {
        console.warn(`amountSum组件[${this.fieldName}]计算时发生异常错误`, e);
      }
      return sumVal;
    },
    showInFormCalc() {
  ...
  mounted() {
    if (this.sumVal) {
      ...
      this.updateHiddenFields({
        sumCostCn: cashToChinese(this.moduleValue)
      })
    }
  },
  methods: {
