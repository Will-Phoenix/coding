...
export default {
  ...
  props: {
    ...
    // 当 multiple 为true 且 limit>=1 时, 有效
    limit: {
      type: Number,
      default: null
    },
    ...
    handleNodeClick(data) {
      ...
      if (this.multiple) {
        ...
        if (this.limit === 1) {
          ...
          return;
        } else if (this.limit > 1 && this.selectedIds.length >= this.limit) {
          return this.$message.warning(`最多选择${this.limit}条数据`)
        }
        ...
        for (let j = 0; j < item.length; j++) {
          inner: for (let ii = 0; ii < this.allList.length; ii++) {
            if (item[j] === this.allList[ii].id) {
              textItem.push(this.allList[ii].fullName)
              break inner
            }
          }
        }
