<template>
  <div class="popupSelect-container">
    ...
    <el-dialog title="选择部门" :close-on-click-modal="false" :visible.sync="visible"
...
import { getDepartmentList } from "@/api/permission/department";
...
export default {
  ...
  methods: {
    ...
    getCurrOrgDeptList() {
      getDepartmentList(this.userInfo.companyId).then(res => {
        this.treeData = res.data.list.map((v) => {
          return {
            ...v,
            type: "department"
          }
    ...
    openDialog() {
      ...
      if (this.selectType === 'custom' || this.selectType === 'org') {
        this.getData()
        this.$nextTick(() => {
          this.setDefault()
        })
