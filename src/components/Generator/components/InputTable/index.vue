<template>
  <div class="jnpf-table-box" :class="[config.__config__.type]">
      ...
      <el-table-column width="50" align="center" label="序号">
        ...
        <template slot-scope="scope">
          <div class="row-action">
            <span class="index" :class="{'btn-disabled':disabled || disableEdit}"> {{ scope.$index + 1 }}</span>
          ...
          <template slot-scope="scope">
            ...
            <template v-if="['select', 'checkbox','radio'].includes(head.__config__.jnpfKey)">
              <el-select v-model="tableFormData[scope.$index][cIndex].value"
                ...
                <el-option v-for="(opt,oIndex) in (head.options || head.__slot__.options || tableFormData[scope.$index][cIndex].options)"
            ...
            <template v-else-if="head.__config__.jnpfKey==='treeSelect'">
              <JNPF-TreeSelect v-model="tableFormData[scope.$index][cIndex].value"
                :options="head.options || tableFormData[scope.$index][cIndex].options" :props="head.props.props"
                :placeholder="head.placeholder" :clearable="head.clearable"
                :multiple="head.multiple" :filterable="head.filterable" :disabled="head.disabled || disableEdit" />
            </template>
            <!-- 级联选择 -->
            <template v-else-if="head.__config__.jnpfKey==='cascader'">
              <el-cascader v-model="tableFormData[scope.$index][cIndex].value"
                ...
                :filterable="head.filterable" :disabled="head.disabled || disableEdit" />
            </template>
            <!-- 其他 -->
    ...
    <div class="table-actions" @click="addItem()" v-if="!disabled && !disableEdit">
...
export default {
  ...
  created() {
    ...
    const boResult = this.buildOptions()
    if (this.value && this.value.length) {
      this.value.forEach(t => this.addRow(t))
      boResult.then(() => this.autoFixOldData())
    } else {
      // this.addRow()
    }
  },
  ...
  methods: {
      ...
      this.tableData.forEach(cur => {
        ...
        if (dyOptionsList.indexOf(config.jnpfKey) > -1) {
          ...
          if (config.dataType === 'dictionary') {
            ...
            const p = getDictionaryDataSelector(config.dictionaryType).then(res => {
          ...
          if (config.dataType === 'dynamic') {
            ...
            const p = getDataInterfaceRes(config.propsUrl, query).then(res => {
    ...
    autoFixOldData() {
      this.tableFormData.forEach(row => {
        row.forEach((col, i) => {
          const jnpfKey = col.jnpfKey || col.__config__.jnpfKey
          if (['cascader', 'treeSelect'].includes(jnpfKey) && col.value instanceof Array && col.value.length > 0) {
            const options = this.tableData[i].options || (this.tableData[i].__slot__ || {}).options || col.options
            this.__dyFixFullPaths(col, col.value, options, this.tableData[i].props.props)
          }
        })
      })
    },
    ...
    getConfById(formId, rowIndex) {
      ...
      for (const key in item) {
        ...
        if (key === 'disabled') {
          newObj[key] = this.disabled || this.disableEdit || item[key]
        }
      }
