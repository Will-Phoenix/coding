      ...
      <div class="transfer__body">
        <div class="transfer-pane">
          ...
          <div class="transfer-pane__body">
            <div>
              <el-tag class="label-tag" @click="getBTreeFilterByLabel()">
                <span class="el-select__tags-text">全部</span>
              </el-tag>
              <el-tag v-for="item in labelTagList" :key="item.id" class="label-tag" @click="getBTreeFilterByLabel(item.id)">
                <span class="el-select__tags-text">{{ item.name }}</span>
              </el-tag>
            </div>
            <el-tree
...
import { getBudgetTreeByLabel, getBudgetLabelList } from "@/api/business/budgetManagement";
...
export default {
  ...
  methods: {
    async getData() {
      // 获取预算树
      await this.getBTreeFilterByLabel();
      // 第一次加载预算树后，判断是否有值（即看申请人所在部门是否有预算），没有预算则不展示该预算组件
      this.$emit("componentShow", this.treeData.length > 0);
      // 加载标签列表
      getBudgetLabelList().then(res => {
        this.labelTagList = res.data;
      });
    },
    treeToList(tree, list) {
      tree.forEach(v => {
        list.push(v);
        if (v.children) {
          this.treeToList(v.children, list);
        }
      });
    },
    async getBTreeFilterByLabel(labelId = undefined) {
      await getBudgetTreeByLabel({ labelId }).then(res => {
        this.treeData = res.data.list;
        const allList = [];
        this.treeToList(this.treeData, allList);
    ...
    handleNodeClick(data) {
      ...
      let item = { ...data }
      // 改造一下：新增limit限制
      if (this.multiple && this.selectedData.length < this.multipleLimit) {
        this.selectedData.push(item);
      } else if (this.multiple && this.selectedData.length >= this.multipleLimit) {
        this.$message.warning("所选部门数量超出限制");
      } else {
    ...
    confirm() {
      if (this.multiple) {
        ...
        this.innerValue = this.selectedData[0].name
