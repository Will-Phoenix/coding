<template>
  <div class="right-board" source="src/components/Generator/index/RightPanel.vue">
        ...
        <el-form v-show="currentTab==='field' && showField" size="small" label-width="90px"
          ...
          <template v-if="activeData.__config__">
            ...
            <template v-if="activeData.__config__.jnpfKey==='card'">
              ...
              <el-form-item>
                <span slot="label">自动隐藏
                  <el-tooltip content="容器内所有子控件全为隐藏的情况下，卡片容器是否自动进行隐藏" placement="top">
                    <a class="el-icon-warning-outline"></a>
                  </el-tooltip>
                </span>
                <el-switch v-model="activeData.__config__.autoShow" />
              </el-form-item>
            </template>
            <!-- row 布局 -->
          ...
          <template v-if="modelType!=3 && modelType!=6 && webType!=3">
            ...
            <template v-if="webType==2">
              <div class="custom-btns-list">
                <draggable :list="formConf.customBtnList" :animation="340" group="customItem"
                           handle=".option-drag">
                  <div v-for="(item, index) in formConf.customBtnList" :key="index"
                       class="custom-item">
                    ...
                    <el-input v-model="item.label" placeholder="按钮名称" size="small">
                      <template slot="append">
                        <el-button type="primary" @click="customBtnEditFunc(item)" class="custom-btn">事件
                        </el-button>
                      </template>
                    </el-input>
                    <div class="close-btn custom-line-icon"
                         @click="formConf.customBtnList.splice(index, 1)">
    ...
    <custom-btn v-if="customBtnVisible" :activeItem="customBtnActiveItem" ref="customBtn"
                @updateCustomBtn="updateCustomBtn" @closeDialog="customBtnVisible=false" />z
  </div>
</template>
...
import CustomBtn from "@/components/ColumnDesign/CustomBtn";
...
export default {
  ...
  methods: {
    ...
    customBtnEditFunc(item) {
      const defaultFunc = "({ formData, request, showMessageAndBack }) => {\n    // 在此编写代码\n    \n}"
      if (!item.event.func) item.event.func = defaultFunc
      this.customBtnActiveItem = item
      this.customBtnVisible = true
      this.$nextTick(() => {
        this.$refs.customBtn.init('pc', item.event, this.webType, this.drawingList, defaultFunc)
      })
    },
    addCustomBtn() {
      const id = this.jnpf.idGenerator()
      this.formConf.customBtnList.push({
        value: 'btn_' + id,
        label: '按钮' + id,
        event: {
          func: ''
    ...
    updateCustomBtn(val) {
      this.formConf.customBtnList.forEach((ele, index) => {
        if (ele.value == this.customBtnActiveItem.value) this.$set(this.formConf.customBtnList[index], 'event', val)
      })
    }
  }
}
