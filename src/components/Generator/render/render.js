...
function buildDataObject(confClone, dataObject, formData) {
  ...
  if (jnpfKey === 'select') {
    if (confClone.__config__.defaultValue && confClone.hasOwnProperty('__slot__') && confClone.__slot__.hasOwnProperty('options')) {
      console.log(confClone.__config__.label, confClone.__config__.defaultValue)
      const filter = confClone.__slot__.options.filter(obj => obj.id === confClone.__config__.defaultValue)
      if (filter.length === 0) {
        const enCodes = confClone.__slot__.options.filter(obj => obj.enCode.toString() === confClone.__config__.defaultValue.toString())
        if (enCodes.length !== 0) {
          console.log(enCodes, confClone.__config__.defaultValue)
          dataObject.props.value = enCodes[0].fullName
        }
      }
    }
  }
  ...
  if (jnpfKey === 'editor') {
    if (dataObject && dataObject.attrs && dataObject.attrs.hasOwnProperty('disabled'))
    dataObject.props.disable = dataObject.attrs.disabled
  }
