...
export default {
  ...
  methods: {
    ...
    getAllList() {
      this.loading = true
      if (this.pagination.keyword) this.nodeId = this.parentId;
      getImUserSelector(this.nodeId, this.pagination).then(res => {
        ...
        this.loading = false
        if (!this.pagination.keyword && this.treeData.length && this.nodeId == this.parentId) {
