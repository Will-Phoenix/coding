...
<template>
  <el-drawer :size="value && isConditionNode() ?'750px':'600px'" class="drawer JNPF-common-drawer"
    ...
    <header slot="title" class="header"
      ...
      <span style="font-size: 12px;margin-left: 10px;">节点ID：{{value.nodeId}}</span>
    </header>
              ...
              <div class="form-item-content" v-if="subFlowForm.launchMsgConfig.on===1">
                ...
                <el-table :data="subFlowForm.launchMsgConfig.templateJson">
                  ...
                  <el-table-column prop="value" label="表单字段">
                    <template slot-scope="scope">
                      <el-select class="variable-box" v-model="it.relationField"
                        placeholder="请选择表单字段" clearable filterable
            ...
            <el-form label-position="top" :model="startForm" class="pd-10-20">
              <el-form-item>
                ...
                <div class="form-item-content">
                  ...
                  <el-button type="primary" style="margin-top: 10px" @click="refreshFieldInfo">刷新配置</el-button>
                </div>
              </el-form-item>
        ...
        <el-tab-pane label="表单权限">
          <div class="form-auth-table">
            <el-table :data="getFormOperates()" class="JNPF-common-table" size="mini" height="100%">
              ...
              <el-table-column prop="id" label="字段名" align="left"></el-table-column>
        ...
        <el-tab-pane label="基础设置" name="config">
          <el-scrollbar class="config-scrollbar">
            <el-form label-position="top" :model="approverForm" class="pd-10-20">
              ...
              <el-form-item label="候选人配置" v-if="approverForm.assigneeType === 7">
                <div class="candidate-row">
                  <div>审批人</div>
                  <el-select style="width: 100px;margin-left: 12px;" v-model="approverForm.candidateConfig.role" placeholder="请选择">
                    <el-option :value="0" label="发起人"></el-option>
                    <el-option :value="1" label="审批人"></el-option>
                    <el-option :value="2" label="全部"></el-option>
                  </el-select>
                  <div style="margin-left: 60px;">是否必选</div>
                  <el-switch style="margin-left: 12px;" v-model="approverForm.candidateConfig.required"></el-switch>
                </div>
              </el-form-item>
          ...
          <el-scrollbar class="config-scrollbar">
            ...
            <el-form class="pd-10-20" style="margin-top:-20px" label-width="90px"
              ...
              <el-form-item>
                ...
                <el-checkbox :disabled="!approverForm.hasOpinion" v-model="approverForm.isOpinionRequire"></el-checkbox>
              </el-form-item>
...
const defaultApproverForm = {
  ...
  hasAgreeRule: true, // 自动同意规则,默认不启用
  agreeRules: [4], // 自动同意规则数组
  formFieldType: 1, // 表单字段审核方式的类型(1-用户 2-部门)
  approvers: [], // 审批人集合
  approverPos: [], // 审批岗位集合
  approverRole: [], // 审批角色集合
    ...
    */
    ...
    getFormFieldList(id, form, isSameForm) {
      getFormInfo(id).then(res => {
        ...
        this[form].moduleJson = res.data.moduleJson
        // 更新所有没设置表单的节点的表单权限
    ...
    refreshFieldInfo() {
      this.startForm.formId && this.getFormFieldList(this.startForm.formId, 'startForm', true)
    }
  },
  watch: {
    visible(val) {
    ...
    'approverForm.hasOpinion'(newVal) {
      if(!newVal){
      this.approverForm.isOpinionRequire = false;
      }
    },
    value(newVal) {
