  ...
  methods: {
      ...
      this.$nextTick(() => {
        ...
        if (this.dataForm.id) {
          ...
          getVisualDevInfo(this.dataForm.id).then(res => {
            ...
            this.sceneData = this.formData.sceneData
            this.sceneFieldData = this.getFormFieldScene()
    ...
    dataFormSubmit() {
      const getData = () => {
        ...
        if (this.dataForm.formData) {
          this.formData.sceneData = this.sceneData ? this.sceneData : null
        }
    ...
    getFormFieldScene() {
      ...
      let list = []
      this.recursionFieldData(fields, list)
      return list
    },
    recursionFieldData(data, arr) {
      data.forEach(obj => {
        if (obj.__config__.hasOwnProperty('children')) {
          this.recursionFieldData(obj.__config__.children, arr)
        } else {
          if (noColumnShowList.indexOf(obj.__config__.jnpfKey) < 0) {
            arr.push(this.fieldModelToMap(obj))
          }
        }
      })
    },
