...
import { APIgetCityAll } from "@/api/cicada/oa";
...
const actions = {
  ...
  getAreaTree({ state, commit }) {
    return new Promise((resolve, reject) => {
      if (!state.areaTree.length) {
        APIgetCityAll().then(res => {
          const arr = res.data;
          forEachTreeData(arr, (item, index, level, parent) => {
            item.label = item.fullName;
            item.value = item.id;
            Object.keys(item).forEach(k => {
              if (!['label', 'value', 'children'].includes(k)) {
                delete item[k];
              }
            });
          });
          commit("SET_AREA_TREE", arr);
          resolve(arr);
        }).catch(error => {
          reject(error);
        });
