...
const actions = {
        ...
        function setData(list) {
          for (let i = 0; i < list.length; i++) {
            ...
            if (e.type == 6) {
              ...
              e.path = location.origin + `/cicada-visual/view/${moduleId}?token=${getToken()}`
            }
            // 外链
            if (e.type == 7) {
              const path = e.urlAddress.replace(/\${dataV}/g, define.dataV).replace(/\${jnpfToken}/g, getToken()).replace(/\${organizeId}/g, userInfo.organizeId);
  ...
  resetToken({ commit, state, dispatch }) {
    return new Promise(resolve => {
      ...
      dispatch('orgInfo/clearOrgAllInfo', null, { root: true })
      commit('SET_MENULIST', [])
      commit('SET_USERINFO', {})
