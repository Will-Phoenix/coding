  ...
  <div class="JNPF-common-layout">
    <div class="content" v-loading="loading">
      <div class="content-flex">
        <div class="left">
          <el-card class="size1">
            <div class="row-flex" style="margin-top: 9px">
              ...
              <el-switch v-if="expenseAccountManage" v-model="statisticData.isPay" :disabled="isPayButton"
                :active-value="1" :inactive-value="0" active-text="已付款" inactive-text="未付款" @change="changeIsPay" />
              <el-button style="font-size: 14px;margin-left: 20px" type="text" :loading="btnLoading"
                v-if="$route.query.financialSystem === 'common' && expenseAccountManage" @click="openAccountVoucher"
                icon="el-icon-edit">编辑记账凭证号</el-button>
              <div v-if="expenseAccountManage">
                <el-upload :action="actionUrl" :headers="uploadHeaders" :show-file-list="false"
                  :on-progress="handleProgress2" :on-success="handleSuccess2" style="margin-left: 10px">
                  <el-button style="font-size: 14px" type="text" :loading="btnLoading2"
                    icon="el-icon-plus">添加新的报销资料</el-button>
                </el-upload>
              </div>
            </div>
            <div style="margin-bottom: 24px">
              <el-tag v-if="detailData.paymentOrderUpload" effect="dark" size="middle"
                class="tag-success">已录入银行付款单</el-tag>
              <el-tag v-else effect="dark" size="middle" class="tag-fail">未录入银行付款单</el-tag>
              <el-tag v-if="detailData.accountingStatus" effect="dark" size="middle" class="tag-success">已记账</el-tag>
              <el-tag v-else effect="dark" size="middle" class="tag-fail">未记账</el-tag>
              <el-tag v-if="statisticData.isPay" effect="dark" size="middle" class="tag-success">已付款</el-tag>
              <el-tag v-else effect="dark" size="middle" class="tag-fail">未付款</el-tag>
              <el-tag v-if="detailData.stickerTicketStatus" effect="dark" size="middle" class="tag-success">已贴票</el-tag>
              <el-tag v-else effect="dark" size="middle" class="tag-fail">未贴票</el-tag>
              <el-tag v-if="detailData.accountingStatus" effect="plain" size="middle" class="tag-plain">记账凭证号 {{
                detailData.voucherYear }}年{{ detailData.voucherMonth }}月记{{ detailData.voucherNo }}号</el-tag>
            </div>
          ...
          <el-card class="size2">
            ...
            <div class="size4-card" style="text-align: center;" v-if="receipts && receipts.length > 0">
              ...
              <el-image v-for="(item, index) in receipts" :key="index" style="max-width: 297mm" :src="item.fidUrl"
          ...
          <el-card class="size3" v-if="applyDataList.length > 0">
            ...
            <div class="size4-card row-flex flex-center">
              <el-image class="small" v-for="item in applyDataList" :key="item.id" :src="item.url"
          ...
          <el-card class="size3" v-if="exsOrderList.length > 0">
            ...
            <div class="size4-card row-flex flex-center">
              <el-image class="small" v-for="item in exsOrderList" :key="item.id" :src="item.url"
                :preview-src-list="[item.url]" />
            </div>
          </el-card>
          <el-card v-if="pictureList.length > 0 || relevantPdf.length > 0">
            ...
            <div class="row-flex">
              <div class="statistic-card size2-card" v-for="(item, index) in pictureList" :key="item.index"
              ...
              <div class="statistic-card size2-card" v-for="(item, index) in relevantPdf" :key="item.index"
        ...
        <div class="right">
          <el-card class="size1">
            <div slot="header" class="row-flex" :style="{ margin: payCertificateList.length > 0 ? '-7px 0' : '' }">
              <div class="sub-title">银行付款证明</div>
              <div v-if="payCertificateList.length > 0" style="color: #00B4C4;font-weight: 500;line-height: 20px;">{{
                payCertificateList.length }}条付款证明</div>
              <div class="text-button">
                <el-upload :action="actionUrl" :headers="uploadHeaders" :show-file-list="false"
                  :on-success="handleSuccess" :on-progress="handleProgress">
                  <el-button v-if="payCertificateList.length > 0 && expenseAccountManage" type="text"
                    :loading="btnLoading3" icon="el-icon-plus" style="font-size: 14px">上传银行付款证明</el-button>
                </el-upload>
              </div>
              <el-button v-if="payCertificateList.length > 0 && expenseAccountManage" type="text" icon="el-icon-delete"
                style="font-size: 14px; margin-left: 10px;" @click="deletePay">删除银行付款证明</el-button>
            </div>
            <div class="row-flex" v-if="payCertificateList.length > 0">
              <div class="statistic-card size2-card" v-for="item in payCertificateList" :key="item.src"
            ...
            <div v-else>
              <el-upload v-if="expenseAccountManage" :action="actionUrl" :headers="uploadHeaders"
                list-type="picture-card" :show-file-list="false" :on-success="handleSuccess">
                <i class="el-icon-plus" style="font-size: 14px;color: #00B4C4;">上传银行付款证明</i>
              </el-upload>
            </div>
          </el-card>
          <el-card class="size4">
            <div slot="header" class="row-flex">
              ...
              <div class="text-button" v-if="expenseAccountManage">
                <el-button type="text" icon="el-icon-plus" style="font-size: 14px"
                  @click="onAddInvoice">添加新的发票</el-button>
              </div>
            </div>
            <div>
              <div class="size3-card" v-for="(item, index) in invoiceList" :key="item.id">
                <embed v-if="item.previewOnlineUrl" style="width: 66%;margin-right: 10px;" :src="item.previewOnlineUrl">
                <el-image v-else style="width: 66%;margin-right: 10px;" :src="item.src"
                  :preview-src-list="[item.src]" />
                <!--                <el-image v-if="item.src" style="width: 66%;margin-right: 10px;" :src="item.src" :preview-src-list="[item.src]" />-->
                <!--                <embed v-else style="width: 66%;margin-right: 10px;" :src="item.previewOnlineUrl">-->
                <div class="invoice-bar black-bar">
                  <div>{{ item.title }}</div>
                  <div v-if="item.previewOnlineUrl" style="cursor: pointer" @click="previewOther(item)">点击放大查看</div>
                  <div v-if="item.isElectronicInvoice === 1" @click="downloadSingleFile(item.title, item.url)"
                    style="cursor: pointer">发票原件下载</div>
                </div>
                <el-image style="flex: 1" v-if="item.checkSrc" fit="cover" :src="item.checkSrc"
                  :preview-src-list="[item.checkSrc]" />
        ...
        <addInvoice :visible.sync="addInvoice.visible" :order-id="orderId" :loading="addInvoice.loading"
          @confirm="onUpdateInvoiceList" />
        <accountVoucher :visible.sync="accountVoucher.visible" :data="accountVoucher.data"
          :loading="accountVoucher.loading" @confirm="updateExsOrderDetail" />
      ...
      <div class="bottom-bar">
        ...
        <div v-if="propertyJson && propertyJson.printShow && detailData.printNum" style="margin: 0 10px;">已打印（{{
          detailData.printNum }}次）</div>
        <el-button type="primary" :loading="btnLoading" @click="downloadAll()">
          批量下载原件
        </el-button>
        <el-button v-if="expenseAccountManage" type="primary" @click="flagToPasted()">
          {{ detailData.stickerTicketStatus === 0 ? "标记为已贴票" : "标记为未贴票" }}
        </el-button>
        <el-button v-if="showNextOrLastButton" style="margin-left: auto" type="primary" @click="goToNextOrLast(-1)">
          上一页
        </el-button>
        <el-button v-if="showNextOrLastButton" type="primary" @click="goToNextOrLast(1)">
          下一页
        </el-button>
      </div>
    </div>
    <HtmlDetailDialog :loading="htmlDetail.loading" :visible.sync="htmlDetail.visible" :html="htmlDetail.data" />
    <optionalBatchPrint :visible.sync="optionalBatchPrint.visible" :data="optionalBatchPrint.data"
      :func="loadPreviewUrl" @confirm="confirmPrint" />
  </div>
</template>

<script>
import { getOtherOrdersDetail, ccdExsOrderTaxStatistic, getExsOrderList } from "@/api/business/expenseAccount";
import { additionPrintListByOrder } from "@/api/business/businessApproval";
import { APIPostCreateOrderPrintRecord, FlowBeforeInfo } from "@/api/workFlow/FlowBefore";
import { getInvoiceDetail, getExsOrderCenterList, stickerTicketStatus } from "@/api/business/pasteTicket";
import { downloadFile } from "@/api/business/signature";
import { saveAs } from "file-saver";
import axios from 'axios';
import JSZip from 'jszip'
import addInvoice from "./components/addInvoice";
import accountVoucher from "./components/accountVoucher";
import previewOnline from "@/components/preview-online/index";
import baseMixin from '@/mixins/print/baseMixin'
import HtmlDetailDialog from './components/HtmlDetailDialog'
import { editExsOrder, batchDelBankSlip } from "@/api/business/expenseAccount";
import { parseJsonObj } from '@/components/Cicada/printHelper'
import { getBudgetDetailOrderPage } from "@/api/business/budgetManagement";
import optionalBatchPrint from "./components/optionalBatchPrint.vue";
import { printPdf } from '@/utils/pdf';

export default {
  components: { HtmlDetailDialog, accountVoucher, previewOnline, addInvoice, optionalBatchPrint },
  mixins: [baseMixin],
  props: {
    propOrderId: {
      type: String,
  ...
  computed: {
    hasElectronicInvoice() {
      return this.invoiceList.filter(v => v.isElectronicInvoice === 1).length > 0;
    },
    hasPaperInvoice() {
      return this.invoiceList.filter(v => v.isElectronicInvoice === 0).length > 0;
    },
    showNextOrLastButton() {
      return this.listQuery && this.queryData && this.expenseAccountManage;
    },
    expenseAccountManage() {
      // 兼容待贴票页面，即from为2，后续待贴票页面废除，只有1
      return this.$route.query.from == 1 || this.$route.query.from == 2;
    },
    propertyJson() {
  ...
  beforeRouteUpdate(to, from, next) {
    ...
    this.listQuery = JSON.parse(to.query.listQuery);
  ...
  created() {
    /* 其中this.$route.query.from  1是报销单管理 2是票据中心 3预算管理 4归档管理*/
    this.orderId = this.$route.query.id || this.propOrderId || "";
    // console.log(this.$route.query.from, "222")
  ...
  methods: {
    ...
    async openOptionalDialog() {
      // console.log(this.OrderId, "openOptionalDialog")
      this.btnLoading = true;
    ...
    finishPrint(error) {
      ...
      const that = this;
      APIPostCreateOrderPrintRecord({ orderId: this.orderId }).then(() => {
        that.$set(this.detailData, 'printNum', (this.detailData.printNum || 0) + 1);
      });
      if (this.detailData.id) {
        this.$confirm('是否将该报销单，标记为已贴票?', '提示', {
          type: 'warning'
        }).then(() => {
          stickerTicketStatus({ ids: [this.detailData.id], status: 1 }).then(() => {
            this.$message.success("操作成功");
            this.detailData.stickerTicketStatus = 1;
          });
        });
      }
    },
    ...
    // 标记为已贴票
    flagToPasted() {
      stickerTicketStatus({ ids: [this.detailData.id], status: this.detailData.stickerTicketStatus ? 0 : 1 }).then(() => {
        this.$message.success("标记成功");
        this.detailData.stickerTicketStatus = this.detailData.stickerTicketStatus ? 0 : 1;
      });
    },
    ...
    openAccountVoucher() {
      Object.assign(this.accountVoucher, { visible: true, loading: false, data: { id: this.orderId, voucherYear: this.detailData.voucherYear, voucherMonth: this.detailData.voucherMonth, voucherNo: this.detailData.voucherNo, accountingStatus: 1 } })
    },
    handleProgress() {
    ...
    handleSuccess(response) {
      try {
        let ids = "";
        if (this.payCertificateList.length === 0) {
          ids = this.payCertificateList.map(v => v.id).join(",").concat(`${response.data.id}`);
        } else {
          ids = this.payCertificateList.map(v => v.id).join(",").concat(`,${response.data.id}`);
        }
        editExsOrder({ id: this.orderId, paymentOrderUpload: 1, paymentOrderFid: ids }).then(async () => {
          this.$message.success("操作成功");
          this.getCcdExsOrderTaxStatistic();
          await this.updateExsOrderDetail();
          this.getCertificateList();
    ...
    // 清空银行证明
    deletePay() {
      this.$confirm('确定要删除银行回执单？删除后数据无法恢复，请确认！', '提示', {
        type: 'warning'
      }).then(() => {
        batchDelBankSlip([this.detailData.id]).then(async () => {
    ...
    handleSuccess2(response) {
      ...
      try {
        ...
        if (this.detailData.exsFids) {
          ids = this.detailData.exsFids.concat(`,${response.data.id}`);
        } else {
          ids = response.data.id;
        }
        editExsOrder({ id: this.orderId, exsFids: ids }).then(async () => {
          this.$message.success("新增成功，请在下方相关资料处查看");
          this.getCcdExsOrderTaxStatistic();
          await this.updateExsOrderDetail();
    ...
    onAddInvoice() {
      this.addInvoice.visible = true
    },
    // 更新是否付款
    changeIsPay(value) {
      this.isPayButton = true;
      editExsOrder({ id: this.orderId, isPay: value ? 1 : 0 }).then(() => {
        this.$message.success("操作成功");
        this.isPayButton = false;
      });
    },
    // 更新报销单详情
    async updateExsOrderDetail() {
      const { formData } = await this.loadFormDetail({ token: this.$store.getters.token, formId: this.orderId });
      this.detailData = formData;
    },
    // 刷新发票列表
    async onUpdateInvoiceList() {
    ...
    async getCurrentPageList() {
      const method = this.$route.query.from == 1 ? getExsOrderList : this.$route.query.from == 2 ? getExsOrderCenterList : this.$route.query.from == 3 ? getBudgetDetailOrderPage : undefined;
      ...
      if (this.pageList.length > 0 && this.pageIndex + flag > this.pageList.length - 1) {
        ...
        this.$router.push({
          ...
          query: {
            ...
            financialSystem: this.$route.query.financialSystem, // YouBIP是用友V8财务系统
