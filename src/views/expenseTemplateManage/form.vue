...
export default {
  ...
  data() {
    return {
      ...
      workflowOptions: [{ "fullName": "选项一", "id": "1" }, { "fullName": "选项二", "id": "2" }],
      templateOptions: [{ "fullName": "选项一", "id": "1" }, { "fullName": "选项二", "id": "2" }],
      sobIdOptions: [{ "name": "选项一", "id": "1" }, { "name": "选项二", "id": "2" }],
    ...
    dataFormSubmit() {
      this.$refs['elForm'].validate((valid) => {
        if (valid) {
          if (this.dataForm.id) {
            // 把基础打印模版id对应的name也返回
            const basePrintDevName = this.templateOptions.filter(v => v.id === this.dataForm.basePrintdevId)[0].fullName;
            this.visible = false;
            this.$emit("confirm", { ...this.dataForm, basePrintDevName });
            return;
          }
