<template>
  <div class="JNPF-common-layout">
    <div class="JNPF-common-layout-center">
        ...
        >
          ...
          >
            <template slot-scope="scope">
              <span v-if="scope.row.channel * 1 === 5">省票池</span>
              <span v-else-if="scope.row.channel * 1 === 3">金财慧税</span>
              <span v-else-if="scope.row.channel * 1 === 2">邮箱票池</span>
              <span v-else-if="scope.row.channel * 1 === 1">财务上传</span>
              <span v-else>用户上传</span>
            </template>
          </el-table-column>
          <el-table-column
            prop="code"
      ...
      <invoiceDetailDialog :visible.sync="detail.visible" :editable="false" :data="detail.data" />
...
import batchInvoice from "@/views/pasteTicketCenter/component/batchInvoice.vue";

export default {
  name: "InvoiceTicketPool",
  components: { batchInvoice, GpySelector, holderRefDialog, GpyDialog, invoiceDetailDialog, invoiceCheckDialog },
  data() {
    return {
      list: [],
  ...
  methods: {
    ...
    initData() {
      ...
      getEntInvoicePage(_query).then(res => {
