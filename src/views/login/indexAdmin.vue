<template>
  <div class="login-container">
    ...
    <div class="login-left">
      <!--<a class="login-company-logo" target="_blank" href="https://www.gantangtech.cn">
        <img class="login-company-logo-img" src="@/assets/images/login-company-logo.png" alt="">
      </a>-->
      <img class="login-banner" src="@/assets/images/login-banner2.png" alt="">
      <div class="login-left-txt" v-if="showTxt">
        ...
        <!--        <a class="link" target="_blank" href="https://www.gantangtech.cn">了解更多</a>-->
      </div>
    </div>
    <div class="login-content">
      <div class="login-form">
        ...
        <div v-show="!isSso && !ssoLoading">
          <div style="display: none" class="login-tab" :class="'active'+active">
            <a
              class="item"
          ...
          <el-form
            ...
            label-position="left"
          >
            <div class="login-form-container">账户密码登录</div>
            ...
            <div @click="active=2" class="login-note-btn">微信扫码登录</div>
          ...
          <div v-show="active==2" class="login-form-QRCode">
            <div v-if="qrcode" class="qrcode-container">
              ...
              <div>
                <img class="qrcode-img" :src="'data:image/png;base64,'+qrcode.qrcodeBuffer">

              </div>
              <div class="qrcode-refresh" @click="onRefreshQrcode" v-if="isQrcodeOvertime">
                <i
              ...
              <div @click="active=1" class="login-note-btn">使用账号密码登录</div>
            </div>
          </div>
        </div>
        <el-button
    ...
    <el-dialog
      ...
      @open="onOpen"
    >
      <iframe width="100%" height="100%" :src="ssoUrl" frameborder="0" v-if="show" />
    </el-dialog>
    <el-dialog
...
<script>
import { getConfig, getQrcodeScene, login } from '@/api/user'
...
export default {
  ...
  methods: {
    onRefreshQrcode() {
      this.isQrcodeOvertime = false;
      clearInterval(this.loopTimer);
      this.loadQrcode();
    },
    loadQrcode() {
      getQrcodeScene().then(({ data }) => {
        this.qrcode = data;
        this.loopQrcodeStatus()
      }).catch(() => {
        this.qrcode = null;
    ...
    loopQrcodeStatus() {
      ...
      this.loopTimer = setInterval(() => {
        login(qs.stringify({ scene: this.qrcode.scene, grant_type: "pcqrcode" })).then(({ data }) => {
          if (data.status === 2) {
            // 扫码未登录
            console.log('扫码未登录*****')
          ...
          if (data.status === 3) {
            // 已登录
            this.$store.dispatch('user/setToken', data).then(res => {
              this.$router.push({
                path: this.redirect || '/appDashbordHome/index',
    ...
    getLoginConfig() {
      getLoginConfig().then(res => {
        ...
        ls.setItem('useSocials', this.socialsList.length)
