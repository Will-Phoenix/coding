      ...
      <div class="JNPF-common-layout-main JNPF-flex-main">
        ...
        <JNPF-table v-loading="listLoading" :data="treeList" row-key="id" v-if="refreshTable"
          ...
          <el-table-column prop="enabledMark" label="状态" width="70" align="center">
            <template slot-scope="scope">
              <el-tag :type="scope.row.enabledMark * 1 === 1 ? 'success' : 'danger'" disable-transitions>
                {{ scope.row.enabledMark * 1 === 1 ? '启用' : '禁用' }}</el-tag>
            </template>
          </el-table-column>
          <el-table-column label="操作" width="150">
            <template slot-scope="scope">
              <tableOpts @edit="addOrUpdateHandle(scope.row.id,scope.row.type,scope.row.parentId)"
                ...
                >{{ scope.row.enabledMark * 1 === 1 ? '禁用' : '启用' }}
                </el-button>
                <el-dropdown>
...
<script>
import { getOrganizeList, delOrganize, updateOrganizeState } from '@/api/permission/organize'
...
export default {
  ...
  methods: {
    ...
    enableOrDisable(row) {
      updateOrganizeState(row.id).then(res => {
        this.$message({
          type: 'success',
          message: res.msg,
          duration: 1500,
