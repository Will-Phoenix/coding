    ...
    <div class="main" v-loading="loading">
      <el-row type="flex" justify="center" align="middle" v-if="!activeStep" class="basic-box">
        <el-col :xs="12" :sm="12" :md="12" :lg="12" :xl="10" class="basicForm">
          ...
          >
            ...
            <el-form-item label="新模版" prop="isNewTemplate">
              <el-switch :disabled="!!dataForm.id" v-model="dataForm.isNewTemplate" :active-value="1" :inactive-value="0" />
            </el-form-item>
      ...
      <template v-if="activeStep==1 && showPrint">
        <print-templater
          ...
          :type="dataForm.type"
        />
        <TemplateDesigner :page-param="pageParam" :tree-data="treeData" ref="templateDesigner" v-if="dataForm.isNewTemplate === 1" :data-form="dataForm" />
      </template>
    </div>
  </el-dialog>
</template>
...
import TemplateDesigner from './template-designer/index'
import { mapGetters } from "vuex";
import { getOrganizeTree } from "@/api/permission/organize";
import { STORAGE_KEY } from "./template-designer/hook"
export default {
  components: { PrintTemplater, TemplateDesigner },
  data() {
    return {
      visible: false,
      loading: false,
  ...
  methods: {
    ...
    async dataFormSubmit() {
      ...
      if (this.dataForm.isNewTemplate === 1) {
        if (this.$refs.templateDesigner.size) {
          let type = '2';
          let direction = '横向';
          const size = this.$refs.templateDesigner.size;
          if (['297_210', '210_297'].includes(size)) {
            type = '2'
            if (size === '210_297') {
              direction = '纵向'
            }
          }
          if (['210_148', '148_210'].includes(size)) {
            type = '3'
            if (size === '148_210') {
              direction = '纵向'
            }
          }
          this.dataForm.pageParam = JSON.stringify({
            mt: "0",
            mb: "0",
            ml: "0",
            mr: "0",
            direction: direction,
            type: type,
            width: this.$refs.templateDesigner.sizeObj.width,
            height: this.$refs.templateDesigner.sizeObj.height
          });
          this.dataForm.printTemplate = await this.$refs.templateDesigner.getHtml();

          if (!this.dataForm.printTemplate) {
            this.btnLoading = false
            return
          }
        } else {
          this.$message.error('请先新建模版')
          this.btnLoading = false
          return
        }
      }
      ...
      }).catch(() => { this.btnLoading = false }).finally(() => { this.btnLoading = false })
    },
    delSql(i) {
