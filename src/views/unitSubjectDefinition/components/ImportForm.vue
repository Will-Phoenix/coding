<template>
  <el-dialog
    ...
    class="JNPF-dialog JNPF-dialog_center JNPF-dialog-import"
    lock-scroll
    width="1000px"
  >
    ...
    <div class="import-main" v-show="active==2">
      <JNPF-table v-loading="listLoading" :data="list">
        <el-table-column prop="account" label="序号" width="150">
          <template slot-scope="scope">
            <el-input v-model="scope.row.account" />
          </template>
        </el-table-column>
        <el-table-column prop="account" label="科目编号" width="150">
          <template slot-scope="scope">
            <el-input v-model="scope.row.account" />
          </template>
        </el-table-column>
        <el-table-column prop="realName" label="科目名称" width="150">
          <template slot-scope="scope">
            <el-input v-model="scope.row.realName" />
          </template>
        </el-table-column>
        <el-table-column prop="gender" label="级数" width="150">
          <template slot-scope="scope">
            <el-input v-model="scope.row.gender" />
          </template>
        </el-table-column>
        <el-table-column prop="email" label="科目属性" width="150">
          <template slot-scope="scope">
            <el-input v-model="scope.row.email" />
          </template>
        </el-table-column>
        <el-table-column prop="organizeId" label="余额方向" width="150">
          <template slot-scope="scope">
            <el-input v-model="scope.row.organizeId" />
          </template>
        </el-table-column>
        <el-table-column prop="managerId" label="助记码" width="150">
          <template slot-scope="scope">
