...
<script>
import { getCcdUserFolderPage, createCcdUserFolder, deleteCcdUserFolder } from "@/api/business/userFolder";
...
export default {
  name: 'UserFolderManage',
  components: { previewOnline, GpySelector, GpyDialog, holderRefDialog, editForm },
  data() {
    return {
      list: [],
      listLoading: false,
  ...
  methods: {
    ...
    initData() {
      ...
      getCcdUserFolderPage(_query).then(res => {
    ...
    handleSuccess(response) {
      ...
      // }
      createCcdUserFolder({ fid: response.data.id }).then(() => {
        this.$message.success("上传成功");
