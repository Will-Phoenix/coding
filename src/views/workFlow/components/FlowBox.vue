<template>
  ...
  <div class="JNPF-preview-main flow-form-main" source="src/views/workFlow/components/FlowBox.vue">
    <div class="JNPF-common-page-header">
      ...
      <template v-if="!loading||title">
        ...
        <div class="flow-urgent-value" v-show="setting.opType!=='-1'">
          ...
          />
          <span
            :style="{'color':flowUrgentList[selectState].color}"
          >{{ flowUrgentList[selectState].name }}</span>
        </div>
      </template>
      <div class="options" v-if="!subFlowVisible">
        ...
        <el-button
          v-else-if="moreBtnList.length == 1"
          type="primary"
          plain
          @click="eventLauncher(moreBtnList[0].key)"
        >
          {{ moreBtnList[0].label }}</el-button>
        <el-button
    ...
    >
      <div class="approve-result-img" :class="flowTaskInfo.status | flowStatus()" />
    </div>
    <el-tabs class="JNPF-el_tabs" v-model="activeTab">
      ...
      <el-tab-pane
        ...
        name="recordSummary"
      >
        <RecordSummary :id="setting.id" :summary-type="summaryType" ref="recordSummary" />
      </el-tab-pane>
      <el-tab-pane
        label="流程评论"
    ...
    <el-dialog
      :title="eventType==='audit'?'审批通过':'审批退回'"
    ...
    >
      ...
      >
        ...
        <template v-if="properties.hasOpinion || eventType === 'reject'">
          <el-form-item label="审批意见" prop="handleOpinion" :rules="[{ required: properties.isOpinionRequire || eventType === 'reject', message: `请输入审批意见`, trigger: 'change' }]">
            <el-row>
              <el-col :span="20">
                <el-input
              ...
              <el-col :span="4">
                <el-button plain @click="commonWords()" class="commonWords-button" style="margin-left: 10px;">
                  常用语
                </el-button>
              </el-col>
            </el-row>
        ...
        <el-form-item label="手写签名" required v-if="properties.hasSign">
          <div class="sign-main">
            ...
            <div @click="addSign" class="sign-style">
              <i class="icon-ym icon-ym-signature add-sign" />
    ...
    <candidate-form
      ...
      :is-custom-copy="properties.isCustomCopy"
    />
    <error-form :visible.sync="errorVisible" :node-list="errorNodeList" @submit="handleError" />
    <actionDialog
      v-if="actionVisible"
      ref="actionDialog"
      :assign-node-list="assignNodeList"
...
import { waitCondition } from '@/utils/ext'
export default {
  name: 'FlowBox',
  ...
  methods: {
    ...
    getBeforeInfo(data) {
      FlowBeforeInfo(data.id || 0, { taskNodeId: data.taskNodeId, taskOperatorId: data.taskId, flowId: data.flowId }).then(res => {
        ...
        this.initBtnList()
        waitCondition(() => !!this.$refs.form, 50, 10000, true).then(() => {
    ...
    handleMore(e) {
      ...
      if (e == 'reject') return this.eventLauncher('reject')
    ...
    eventReceiver(formData, eventType) {
      ...
      if (eventType === 'audit' || eventType === 'reject') {
        ...
        Candidates(this.setting.taskId, { ...this.formData, formData: this.setting.formData }).then(res => {
          let data = res.data
    ...
    actionReceiver(query) {
      if (!query) {
        query = {
          ...
          signImg: '',
          fileList: [],
          formData: this.setting.formData
        }
      }
      ...
      actionMethod(id, { flowId: this.setting.flowId, ...query }).then(res => {
