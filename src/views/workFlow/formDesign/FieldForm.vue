<template>
  <div class="fieldFrom" source="src/views/workFlow/formDesign/FieldForm.vue">
    ...
    <el-table :data="drawingList" size="mini">
      ...
      <el-table-column prop="filedName" label="名称">
        <template slot-scope="scope">
          <el-input v-model="scope.row.filedName" placeholder="输入名称" />
        </template>
      </el-table-column>
      <el-table-column prop="filedId" label="字段">
        <template slot-scope="scope">
          <el-input v-model="scope.row.filedId" placeholder="输入字段" />
        </template>
      </el-table-column>
    ...
    getData() {
      return new Promise((resolve, reject) => {
        ...
        if (!this.drawingList.length) {
          reject({ msg: '【字段设置】表单列表不允许为空', target: 1 })
          return
        }
    ...
    exist() {
      ...
      for (let i = 0; i < this.drawingList.length; i++) {
        ...
        if (num.length > 1) {
          this.$message({
            showClose: true,
            message: `【字段设置】名称'${e.filedName}'已重复`,
            type: 'error',
            duration: 1000
          });
          isOk = false
